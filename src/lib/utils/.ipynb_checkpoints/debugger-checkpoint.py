...
class Debugger(object):
  ...
  # custom function to calculate rotation angle
  ...
  def calculate_median_angle(self, points):
          angles = []
          for i in range(len(points)):
              for j in range(i+1, len(points)):
                  dx = points[j][0] - points[i][0]
                  dy = points[j][1] - points[i][1]
                  # Compute the angle with the horizontal axis
  ...
  def add_coco_hp(self, points, img_id='default'): 
   ...
   # rotation_angle = calculate_rotation_angle(points, index1, index2)
    print("points:", points)
    #rotation_angle = self.calculate_median_angle(points[:2])  # Use only eye points
    #print("Robust rotation angle:", rotation_angle)
    index1, index2 = 0, 1
    point1 = points[0]
    point2 = points[1]
    rotation_angle = self.calculate_rotation_angle(point1, point2)
    print("Rotation angle:", rotation_angle)

    point1 = points[3]
    point2 = points[4]
    rotation_angle = self.calculate_rotation_angle(point1, point2)
    print("Lip Rotation angle:", rotation_angle)
  ...
  def add_coco_bbox_with_rotation(self, bbox, points, cat, conf=1, show_txt=False, img_id='default'):
    # Convert bbox and points to numpy arrays
    bbox = np.array(bbox, dtype=np.int32)
    points = np.array(points, dtype=np.int32).reshape(self.num_joints, 2)

    # Calculate rotation angle and distance for eyes
    eye_point1 = points[0]  # Assuming 0 is the index for one eye
    eye_point2 = points[1]  # Assuming 1 is the index for the other eye
    eye_rotation_angle = self.calculate_rotation_angle(eye_point1, eye_point2)
    eye_distance = np.linalg.norm(eye_point1 - eye_point2)

    # Calculate rotation angle and distance for lips
    lip_point1 = points[3]  # Assuming 3 is the index for one side of the lips
    lip_point2 = points[4]  # Assuming 4 is the index for the other side of the lips
    lip_rotation_angle = self.calculate_rotation_angle(lip_point1, lip_point2)
    lip_distance = np.linalg.norm(lip_point1 - lip_point2)

    # Define a threshold for "very close" distance
    close_threshold = 10  # This threshold can be adjusted based on your specific needs

    # Decide whether to use eyes, lips, or both for calculating rotation angle
    if eye_distance < close_threshold:
        # Eyes are very close, use only lips for rotation angle
        rotation_angle = lip_rotation_angle
    else:
        # Eyes are not too close, use weighted average of eyes and lips for rotation angle
        total_distance = eye_distance + lip_distance
        eye_weight = eye_distance / total_distance
        lip_weight = lip_distance / total_distance
        rotation_angle = (eye_rotation_angle * eye_weight) + (lip_rotation_angle * lip_weight)

    # If the direction is opposite, negate the angle
    rotation_angle = -rotation_angle  # Invert the angle for correct direction

    print("Rotation angle:", rotation_angle)

    # Calculate the center of the bbox for rotation
    center = ((bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2)
    center = tuple(map(int, center))  # Ensure the center coordinates are integer

    # Get rotation matrix
    rot_mat = cv2.getRotationMatrix2D(center, rotation_angle, 1.0)

    # Apply rotation to the bbox corners
    bbox_corners = np.array([[bbox[0], bbox[1]], [bbox[2], bbox[1]], [bbox[2], bbox[3]], [bbox[0], bbox[3]]])
    rotated_bbox_corners = cv2.transform(np.array([bbox_corners]), rot_mat)[0].astype(int)

    # Define a different color for the head of the bounding box
    head_color = [0, 255, 0]  # Example: green color for the head side

    # Draw rotated bbox with different color for the head side
    cat = int(cat)
    c = self.colors[cat][0][0].tolist()
    if self.theme == 'white':
        c = (255 - np.array(c)).tolist()
    ...
    for i in range(4):
        if i == 0:  # Assuming the first side is the 'head' side
            line_color = head_color
        else:
            line_color = c
        cv2.line(self.imgs[img_id], tuple(rotated_bbox_corners[i]), tuple(rotated_bbox_corners[(i + 1) % 4]), line_color, 2)
