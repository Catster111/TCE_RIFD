...
import numpy as np
import matplotlib
matplotlib.use('agg')  # Ensure the backend is set
...
def _topk(scores, K=40):
    ...
    #topk_scores = [1,1,200]  topk_inds = [1,1,200]
    topk_inds = topk_inds % (height * width)
    #topk_ys   = (topk_inds / width).int().float()
    topk_ys = (torch.true_divide(topk_inds,width)).int().float()  #获得在哪一行
    topk_xs  = (topk_inds % width).int().float()                  #获得在哪一列
    #topk_xs = [1,1,200] topk_ys = [1,1,200]

    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)
    #topk_score = [1,200]  topk_ind = [1,200]
    #topk_clses = (topk_ind / K).int()
    topk_clses = (torch.true_divide(topk_ind,K)).int()   #[1,200]
    topk_inds = _gather_feat(
        topk_inds.view(batch, -1, 1), topk_ind).view(batch, K)   #[1,200]
    topk_ys = _gather_feat(topk_ys.view(batch, -1, 1), topk_ind).view(batch, K)  #[1,200]
    topk_xs = _gather_feat(topk_xs.view(batch, -1, 1), topk_ind).view(batch, K)  #[1,200]

    return topk_score, topk_inds, topk_clses, topk_ys, topk_xs

'''这个是专门为landmark进行设计解释器'''
def _topk_landmark(scores,K=40):
    batch, cat, height, width = scores.size()
    ...
    topk_xs = (topk_inds % width).int().float()
    #top_kps = torch.cat([topk_xs,topk_ys],2)
    #batch,h,w = top_kps.size()
    #top_kps = top_kps.view(1,batch,h*w)
...
def rotate_bbox(bbox, angle, cx, cy):
    # Convert bbox to PyTorch tensor if it's not already
    bbox = torch.tensor(bbox).float()
    cos_a = math.cos(angle)
    sin_a = math.sin(angle)

    # Move bbox to origin, rotate, and move back to center
    new_bbox = []
    for i in range(0, len(bbox), 2):
        x, y = bbox[i] - cx, bbox[i + 1] - cy
        x_new = cos_a * x - sin_a * y + cx
        y_new = sin_a * x + cos_a * y + cy
        new_bbox.extend([x_new, y_new])
...
def centerface_decode(
    ...
    # heat = Heatmap for detecting center points of objects
    # wh = Width and height of the bounding boxes
    # kps =  Keypoints for landmarks on the detected objects
    # reg =  Optional parameter for regression of the center points
    # hm_hp = Optional heatmap for keypoints
    # hp_offset = Optional offset for keypoints
    # K = The number of top detections to consider.
    ...
    #num_joints = kps.shape[1] // 2
    num_joints = kps.shape[1]  #The number of keypoints 
    # heat = torch.sigmoid(heat)

    # perform nms on heatmaps
    
    #NMS is applied to the heatmap to identify the most significant points 
    #(like the centers of detected faces).

    heat = _nms(heat)                  #获得最大值的前两百[1,1,200,200]
    '''

    heatmap = heat.squeeze()  # This will change the shape to [128, 128]
    ...
    '''
    # extracts the top K detections using _topk, which gives scores, indices, classes, and coordinates
    scores, inds, clses, ys_int, xs_int = _topk(heat, K=K)   #ys_int的格式为[1,200]
 
    '''    这仅仅就是进行测试——开始    '''
    '''
    a= torch.randn(200,5,64,64)
    heat_test = _nms(a)
    top_kps = _topk_landmark(heat_test, K=1)  # ys_int的格式为[1,200]
    '''
    '''    这是进行测试的——结束    '''
    # scores, inds, clses, ys_int, xs_int, K = threshold_choose(heat, threshold=0.05)

    # The regression offset is applied to refine the center points of the detected objects.
    # , making the detection more accurate
    if reg is not None:  # 回归的中心点偏移量
        print("The regression offset is processing")
        reg = _tranpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, K, 2) # size of reg.view torch.Size([1, 600, 2]) 2 is x and y 
    ...
    # The function processes the width and height of each bounding box and adjusts them appropriately.
    ...
    '''
    #这个地方就是获得解码的地方
    heat = _nms(heat)  # 获得最大值的前两百
    scores, inds, clses, ys_int, xs_int = _topk(heat, K=K) 获得最大值的关键点
    '''

    '''这就是以前的landmark的解码方式（回归）  start'''
    '''
    ...
    '''新版的landmark 的修改的文件 start'''

    # Keypoint Processing:
    # - The keypoints are also processed through NMS and other transformations to map them from polar to Cartesian coordinates.
    # - Adjustments are made to resize and reposition keypoints based on the bounding box dimensions.
    #修改版本
    kps = _nms(kps)
    topk_ys,topk_xs = _topk_landmark(kps,K=1)
    '''这部分用于从极坐标变换恢复到笛卡尔积坐标系下的一些具体的数据'''
    #用于计算r的大小
    kps_batch, kps_cat, kps_height, kps_width = kps.size()
    r = (math.sqrt(math.pow(kps_height/2, 2) + math.pow(kps_width/2, 2)) / kps_height)
    theta = 2*math.pi/kps_height
    topk_xs_theta = topk_xs.clone()
    topk_ys_r = topk_ys.clone()
    topk_xs_theta = topk_xs_theta * theta #生成真实的theta弧度
    topk_ys_r = topk_ys_r * r     #生成真实的r距离
    #以下的代码用于生成笛卡尔坐标下的距离
    topk_xs = (topk_ys_r * torch.cos(topk_xs_theta)).view(batch,K,num_joints)
    topk_ys = (topk_ys_r * torch.sin(topk_xs_theta)).view(batch,K,num_joints)
    #这个部分是进行resize的
    #获得要进行变换的scale
    pred_wh = (pred_wh/kps_height).expand(batch,K,num_joints)
    topk_xs = (topk_xs * pred_wh).view(K,num_joints,1)
    topk_ys = (topk_ys * pred_wh).view(K,num_joints,1)

    #用于处理预测出结果的格式形状
    top_kps = torch.cat([topk_xs,topk_ys],2)
    kps = top_kps.view(batch,K,num_joints * 2)
    ...
    '''end'''
    '''
    print("kps",kps.shape[1])
    keypoint_indices = [0, 1]  # Adjust these indices based on your keypoints structure
    print("bboxes.shape[1]",bboxes.shape[1])
    for i in range(bboxes.shape[1]):
        bbox = bboxes[0, i, :].clone()  # Assuming batch size of 1 for simplicity
        p1 = kps[0, i, keypoint_indices[0]*2:keypoint_indices[0]*2+2]
        p2 = kps[0, i, keypoint_indices[1]*2:keypoint_indices[1]*2+2]
        ...
        rotated_bbox = rotate_bbox(bbox, 100, cx, cy)
        bboxes[0, i, :] = torch.tensor(rotated_bbox)
    '''
    # Further Processing with Heatmap for Keypoints
    # This section involves additional processing if a separate heatmap for keypoints (hm_hp) is available. 
    # It adjusts keypoints based on this heatmap, considering various factors like bounding box coordinates and distances.
    ...
    if hm_hp is not None:
        ...
        print("test")
    detections = torch.cat([bboxes, scores, kps, clses], dim=2)  # box:4+score:1+kpoints:10+class:1=16[]
    #detections = torch.cat([bboxes], dim=2)
    '''
        roialign测试代码 start
    '''
    '''
    ...
    inputTensor = torch.rand(2, 24, 128, 128).cuda ()
    pooler = torchvision.ops.RoIAlign(output_size=64, sampling_ratio=2, spatial_scale=5)
    output = pooler(inputTensor, boxes)  #   [200,1,64,64] batch
    '''
    '''
        roialign测试代码 end
    '''
