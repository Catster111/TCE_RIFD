...
class LightHybridUp(nn.Module):
    ...
    def forward(self, layers):
        ...
        # 2. ทำ Feature Fusion จากลึกไปตื้น *** แก้ไขให้ทำงานครบทุก stage ***
        num_fusion_stages_to_run = len(layers) - 1 # ควรจะเป็น 3 สำหรับ input 4 features
    
        for i in range(num_fusion_stages_to_run): # ลูปจะทำงานสำหรับ i = 0, 1, 2
            ...
            # print(f"After fusion step {i}: {x.shape}") # i=0 -> s16, i=1 -> s8, i=2 -> s4
...
class LightweightHybridModel(nn.Module):
    # เพิ่ม argument สำหรับชื่อ backbone และสถานะ pretrain
    def __init__(self, heads, head_conv=64, backbone_name='mobilenetv3_large_100', pretrained=True): # <--- เปลี่ยน default backbone และ head_conv
        super().__init__()
        self.heads = heads
        print(f"Initializing LightweightHybridModel with Backbone: {backbone_name}, Pretrained: {pretrained}")
        print(f"Neck output dimension (head_conv): {head_conv}")

        # --- สร้าง Backbone ด้วย TIMM ---
        # *** ต้องตรวจสอบ out_indices ให้ตรงกับ stride 4, 8, 16, 32 ของ 'backbone_name' ที่เลือก ***
        # ตัวอย่าง Indices (อาจจะไม่ถูกต้องสำหรับทุกโมเดล ต้องเช็ค!)
        # สำหรับ MobileNetV3 Large อาจจะเป็น (1, 2, 4, 6) หรือ (0,1,2,3) ถ้า feature_info เริ่มจาก stride 4?
        # สำหรับ EfficientNet อาจจะเป็น (1, 2, 3, 4) หรือ (0,1,2,3)
        # **จำเป็นต้องตรวจสอบกับเอกสาร timm หรือลอง print self.backbone.feature_info**
        try:
            self.backbone = timm.create_model(
                backbone_name,
                pretrained=pretrained,
                features_only=True,
                out_indices=(1, 2, 3, 4) # <--- *** ค่า Indices สมมติ ต้องตรวจสอบ ***
            )
            backbone_channels = self.backbone.feature_info.channels()
            backbone_strides = self.backbone.feature_info.reduction()
            print(f"Using TIMM Backbone. Features - Strides: {backbone_strides}, Channels: {backbone_channels}")
            if len(backbone_channels) != 4:
                raise ValueError(f"TIMM model '{backbone_name}' with specified indices did not return 4 feature maps.")
        except Exception as e:
            print(f"Error creating TIMM backbone '{backbone_name}': {e}")
            print("Please check model name, indices, and timm installation.")
            # อาจจะ raise Error หรือ fallback ไปใช้โมเดลพื้นฐานถ้าต้องการ
            raise e
        # ---------------------------------

        # --- Transformer (เหมือนเดิม แต่ input_dim เปลี่ยนตาม backbone ใหม่) ---
        self.transformer = LightTransformer(
            input_dim=backbone_channels[-1], # ใช้ channel ของ feature ลึกสุด
            depth=3, num_heads=4, key_dim=32, attn_ratio=2
        )

        # --- FPN Neck (รับ channels จาก backbone ใหม่) ---
        # อย่าลืมแก้ Activation ใน IDAUp / LightHybridUp ให้เป็น SiLU ถ้าต้องการ
        self.fpn = LightHybridUp(backbone_channels, out_dim=head_conv)

        # --- Head และ Norm (เหมือนเดิม) ---
    ...
    def forward(self, batch):
        ...
        # --- ดึง features จาก TIMM backbone (จะได้เป็น List/Tuple) ---
        ...
        hybrid_features = list(features[:-1]) + [transformed_feature]
        ...
        ret = self.head(x, batch) # ยังคงส่ง batch ถ้า Head ต้องการ

        # return ret # แนะนำให้คืน dict โดยตรง
        return [ret] # หรือคงไว้ถ้าจำเป็น

# Function to create the lightweight hybrid model
# Function to create the model
def get_lightweight_hybrid_net(heads=None, head_conv=64, backbone_name='mobilenetv3_large_100', pretrained=True, **kwargs):
    # Removed output_size, added backbone_name, pretrained
    if 'num_layers' in kwargs:
        print(f"Note: num_layers parameter is ignored for hybrid model")

    model = LightweightHybridModel(heads, head_conv=head_conv, backbone_name=backbone_name, pretrained=pretrained)
    return model
...
if __name__ == '__main__':
    ...
    # Create model
    model = get_lightweight_hybrid_net({'hm': 1, 'hm_offset': 2, 'wh': 2, 'landmarks': 10}, head_conv=24)
