...
class MultiPoseDataset(data.Dataset):  # 将[x1,y1(左下角的坐标)，w,h]-->[x1,y1,x2,y2]
    ...
    def __getitem__(self, index):
        ...
        temp_path = '/usr/src/app/RPNet/temp/'
        img, anns = Data_anchor_sample(img, anns)

           # --- >>> START: ส่วนปรับ Bbox ตาม Face Center (ก่อน Rotation) <<< ---
        adjusted_bboxes = []
        original_pts_list = [] # Use different name to avoid conflict later
        original_face_centers_list = []
        valid_obj_indices = []

        for k_adjust in range(num_objs):
             ann = anns[k_adjust]
             bbox_orig = self._coco_box_to_bbox(ann['bbox']) # [x1, y1, x2, y2] original
             pts_orig = np.array(ann['keypoints'], np.float32).reshape(self.num_joints, 3)
             face_center_orig = ann.get('face_center', None)

             # Store original points and face center
             original_pts_list.append(pts_orig)
             original_face_centers_list.append(face_center_orig)

             # Adjust bbox center if face_center is valid
             current_bbox_to_rotate = bbox_orig.copy() # Start with original
             if face_center_orig is not None and np.isfinite(face_center_orig).all():
                 w_o = bbox_orig[2] - bbox_orig[0]
                 h_o = bbox_orig[3] - bbox_orig[1]
                 if w_o > 0 and h_o > 0:
                      fc_x, fc_y = face_center_orig
                      adj_x1 = fc_x - w_o / 2
                      adj_y1 = fc_y - h_o / 2
                      adj_x2 = fc_x + w_o / 2
                      adj_y2 = fc_y + h_o / 2
                      current_bbox_to_rotate = np.array([adj_x1, adj_y1, adj_x2, adj_y2], dtype=np.float32)
                      # print(f"Adjusted bbox {k_adjust} using face_center.") # Optional debug
                 # else: keep current_bbox_to_rotate as bbox_orig
             # else: keep current_bbox_to_rotate as bbox_orig

             adjusted_bboxes.append(current_bbox_to_rotate)
             valid_obj_indices.append(k_adjust) # Assume all are valid for now

        # Filter if needed (though currently keeping all)
        if len(valid_obj_indices) != num_objs:
             # This block shouldn't run with current logic, but keep for safety
             anns = [anns[i] for i in valid_obj_indices]
             adjusted_bboxes = [adjusted_bboxes[i] for i in valid_obj_indices]
             original_pts_list = [original_pts_list[i] for i in valid_obj_indices]
             original_face_centers_list = [original_face_centers_list[i] for i in valid_obj_indices]
             num_objs = len(anns)
             if num_objs == 0: print(f"Warning: No valid objects remaining after adjustment for img_id {img_id}.")
        ...
        try:
            ...
            print(f"Skipping rotation due to error: {e}")
            rot_mat, img = np.eye(2, 3), img  # Identity matrix for no rotation
