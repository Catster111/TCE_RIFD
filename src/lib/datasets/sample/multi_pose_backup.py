...
class MultiPoseDataset(data.Dataset):  # 将[x1,y1(左下角的坐标)，w,h]-->[x1,y1,x2,y2]
    ...
    def _rotate_img_bboxes(self, img, angle=5, scale=1.):
        MAX_DIMENSION = 8192  # Define a maximum allowable dimension
        MAX_ATTEMPTS = 5  # Maximum number of attempts to reduce angle and scale
        ...
        attempts = 0
        while (nw > MAX_DIMENSION or nh > MAX_DIMENSION) and attempts < MAX_ATTEMPTS:
            angle /= 2
        ...
        if nw > MAX_DIMENSION or nh > MAX_DIMENSION:
            raise ValueError(f"Image dimensions {nw}x{nh} exceed the limit after rotation")
