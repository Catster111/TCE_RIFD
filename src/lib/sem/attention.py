...
import torch.nn.utils.spectral_norm as spectral_norm_fn
import torch.nn.utils.weight_norm as weight_norm_fn
from PIL import Image
from torchvision import transforms
from torchvision import utils as vutils
from sem import common
from sem.utils.tools import extract_image_patches,\
    reduce_mean, reduce_sum, same_padding

#in-scale non-local attention

class NonLocalAttention(nn.Module):
    def __init__(self, channel=64, reduction=2, ksize=3, scale=1, stride=1, softmax_scale=2, average=True, conv=common.default_conv):
    ...
    def attention(self, x_embed_1, x_embed_2, x_assembly):
        N, _, H, W = x_embed_1.shape
        x_embed_1 = x_embed_1.view(N, -1, H*W)
        x_embed_2 = x_embed_2.view(N, -1, H*W)
        x_assembly = x_assembly.view(N, -1, H*W)

        score = torch.bmm(x_embed_1.transpose(1, 2), x_embed_2)
        score = F.softmax(score, dim=-1)

        x_final = torch.bmm(score, x_assembly.transpose(1, 2))
        return x_final.view(N, -1, H, W)
    ...
    def forward(self, input):
        ...
        x_final = torch.utils.checkpoint.checkpoint(self.attention, x_embed_1, x_embed_2, x_assembly)
