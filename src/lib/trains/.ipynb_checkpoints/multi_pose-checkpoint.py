...
class MultiPoseLoss(torch.nn.Module):
  def __init__(self, opt):
    ...
    self.crit = FocalLoss()                     #FocalLoss
    self.crit_hm_hp = torch.nn.MSELoss() if opt.mse_loss else FocalLoss()    #FocalLoss
    self.crit_kp = RegWeightedL1Loss() if not opt.dense_hp else \
                   torch.nn.L1Loss(reduction='sum')                          #L1
    self.crit_reg = RegL1Loss() if opt.reg_loss == 'l1' else \
                    RegLoss() if opt.reg_loss == 'sl1' else None             #regLoss
    self.opt = opt
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks):
      ...
      if opt.eval_oracle_hmhp:   #no
        output['hm_hp'] = batch['hm_hp']
      if opt.eval_oracle_hm:     #跳过
        output['hm'] = batch['hm']
      if opt.eval_oracle_kps:  #跳过
        if opt.dense_hp:
      ...
      if opt.eval_oracle_hp_offset:  #跳过
      ...
      if opt.reg_offset and opt.off_weight > 0:
        off_loss += self.crit_reg(output['hm_offset'], batch['reg_mask'],             # 3. 人脸bbox中心点下采样，所需要的偏差补偿（reg偏差）
      ...
      if opt.dense_hp:            #跳过
        ...
        '''
        lm_loss += self.crit_kp(output['landmarks'], batch['hps_mask'],               # 4. 关节点的偏移，这个地方使用的RegWeightedL1Loss(),这个地方要修改成为
                                batch['ind'], batch['landmarks']) / opt.num_stacks
        '''
        #使用的是FocalLoss
        test = batch['hm_hp_one'].reshape((-1,5,64,64))
        lm_loss += self.crit_hm_hp(output['landmarks'], test) / opt.num_stacks
