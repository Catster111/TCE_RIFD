...
class MultiPoseDetector(BaseDetector):
  ...
  # Performs pose detection on the input images.
  ...
  # Converts detections to a NumPy array and reshapes it.
  ...
  # Merges detections from different sources or scales.
  # Applies non-maximum suppression (NMS) if necessary.
  ...
  # Adds the original image and detected bounding boxes and keypoints to the debugger.
  def show_results(self, debugger, image, results):
    debugger.add_img(image, img_id='multi_pose')
    
    for bbox in results[1]:
      if bbox[4] > self.opt.vis_thresh:
        # require bbox rotating 
        # Calculate rotation angle from keypoints

        debugger.add_coco_bbox_with_rotation(bbox[:4], bbox[5:39], 0, bbox[4], img_id='multi_pose')
    ...
    save_path = '/home/kunanon.k/Sakon_works/faceDetections/RPNets/output'
    debugger.save_all_imgs(path=save_path, genID=True)
